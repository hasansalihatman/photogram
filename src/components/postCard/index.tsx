...
const PostCard: React.FunctionComponent<IPostCardProps> = ({ data }) => {
  ...
  const updateLike = (isLike: boolean) => async () => {
    setLikesInfo({
      ...likesInfo,
      likes: isLike ? likesInfo.likes + 1 : likesInfo.likes - 1,
      isLike,
    });
    if (isLike) {
    ...
    await updateLikesOnPost(
      data.id!,
      data.userlikes!,
      isLike ? likesInfo.likes + 1 : likesInfo.likes - 1
    );
  };

  if (data.photos && data.photos[0] && data.photos[0].cdnUrl) {
    return (
      <Card className="mb-6">
